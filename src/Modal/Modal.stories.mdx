...
Display a window on top of everything. Use in moderation!
...
By setting the `aside` prop, the modal will now appear from the right.

<Preview>
  <Story name="modal aside">
    {() => {
      const [modalIsOpen, setIsOpen] = React.useState(false);
      return (
        <Box>
          <button onClick={() => setIsOpen((s) => !s)}>open aside</button>
          <Modal
            aside
            title={
